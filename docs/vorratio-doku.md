...
- **Ei ist funktionsbasiert modelliert** (5 Datensätze: Binden · Lockern/Backen · Hauptzutat Rührei/Omelett · Aufschlagen/Eischnee · Ei-Geschmack) – es gibt keinen 1:1-Allrounder. Alle anderen Zutaten zutatenbasiert. Jeder Ei-Datensatz trägt mit `funktion_frage` einen Satz, der die Funktion am Gericht festmacht („Das Ei hält die Masse zusammen: Bratlinge, Frikadellen …"): Im Ersatz-Tab stehen sonst fünf gleich betitelte Karten „Ei" nebeneinander und die Aufteilung wirkt wie eine Dublette statt wie die Frage, die sie ist.
...
- Installation über Safari-Share „Zum Home-Bildschirm" – **immer von der festen Adresse** `https://maxkruggel.github.io/Vorratio/` (GitHub Pages, deployt automatisch bei jedem Push auf `main`). Der lokale Server (`python3 -m http.server`) ist nur zum Entwickeln: Seine Adresse ist eine wechselnde LAN-IP, und der Browser bindet Daten an genau diese Adresse – auf einer neuen IP ist der Vorrat leer.
- **Selbstaktualisierung (umgesetzt):** Eine Homescreen-Web-App wird auf iOS fortgesetzt und nicht neu geladen; ohne aktives Nachsehen bliebe der Stand der Installation für immer stehen. Vorratio fragt darum bei jeder Rückkehr in den Vordergrund beim Server nach (`updateViaCache: "none"`), lässt den neuen Service Worker sofort übernehmen (`skipWaiting`) und lädt sich einmal neu – aber nur, wenn gerade nichts läuft (kein Kochmodus, kein Editor, kein Diktat, kein Scan, kein offener Dialog); sonst wartet das Update auf den nächsten ruhigen Moment. Nach dem Neustart landet man wieder im selben Tab und bekommt eine kurze Rückmeldung. Der Stand ist im Profil unter „App" sichtbar und dort auch von Hand prüfbar. **Neu auf den Homebildschirm legen muss man nie** – und verliert damit auch keine Daten mehr.
- Zwei Fallen, die das vorher verhindert hätten: Der Browser darf `sw.js` bis zu 24 h aus seinem Cache beantworten (deshalb `updateViaCache: "none"`), und die Shell-Dateien liegen auf GitHub Pages zehn Minuten im HTTP-Cache (deshalb `cache: "reload"` beim Installieren und Revalidierung beim Ausliefern).
...
- Auswertung zweigleisig, gleiche Ergebnisform: **lokal** (Zahlwörter, Einheiten, Katalog-Matching mit Einheiten-Schärfung – „zwei Dosen Tomaten" ≠ „zwei Tomaten") oder **über Claude**, wenn ein Key hinterlegt ist.
- **Aufzählungen kommen ohne Satzzeichen an:** Die iOS-Spracherkennung setzt selten Kommas – „Salz Chiliflocken Sonnenblumenkerne" ist ein einziger Block. Der Parser trennt deshalb dreifach: an Satzzeichen und Aufzählungswörtern, an jeder genannten Menge („zwei Kilo Mehl eine Dose Mais"), und innerhalb dessen durch Abgleich von Wortfenstern gegen den Zutatenkatalog. Zustandsangaben hängen am zuletzt genannten Artikel, damit „Milch ist fast leer, Reis halb voll" auch ohne Komma richtig landet. Abgesichert in `tools/test-diktat.mjs` (läuft in der CI). Fällt die API aus, springt der lokale Parser ein; ohne Key ist das Diktat vollständig offline nutzbar.
- Toleranzprinzip: „halb voll", „fast leer", „noch ein Rest" werden als Anteil der Packung gebucht, nicht in Scheingramm. Zutat-IDs aus dem Modell werden gegen Katalog und eigene Artikel geprüft; was sich nicht zuordnen lässt, wird als eigener Artikel angelegt.
- **Der Wortschatz wächst mit dem Vorrat:** Der Zutatenkatalog kennt keine Backoblaten und keine Fertigsuppe – beim Erkennen zählen darum die selbst angelegten Artikel des Haushalts gleichberechtigt mit. Was einmal aufgenommen wurde, wird beim nächsten Diktat wiedererkannt und trennt sich dann auch ohne Satzzeichen sauber von seinen Nachbarn. Weil sich jede Spracherkennung bei ungewohnten Wörtern verhört („Backoblaten" → „Backupladen"), ist der Name in der Bestätigungsliste direkt korrigierbar; einmal geradegetippt, sitzt er dauerhaft.
